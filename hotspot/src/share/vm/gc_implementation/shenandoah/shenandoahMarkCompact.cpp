...
public:
  ...
  void heap_region_do(ShenandoahHeapRegion* r) {
    ...
    assert (r->is_committed(), err_msg("only committed regions in heap now, see region " SIZE_FORMAT, r->index()));
  ...
  void heap_region_do(ShenandoahHeapRegion* r) {
    if (r->is_humongous_start()) {
      ...
      if (!_ctx->is_marked(humongous_obj)) {
        assert(!r->has_live(),
               err_msg("Region " SIZE_FORMAT " is not marked, should not have live", r->index()));
        _heap->trash_humongous_region_at(r);
      } else {
        assert(r->has_live(),
               err_msg("Region " SIZE_FORMAT " should have live", r->index()));
      }
...
void ShenandoahMarkCompact::distribute_slices(ShenandoahHeapRegionSet** worker_slices) {
  ...
  FREE_C_HEAP_ARRAY(size_t, live, mtGC);

#ifdef ASSERT
  BitMap map(n_regions, true /* in_resource_area */);
  for (size_t wid = 0; wid < n_workers; wid++) {
    ShenandoahHeapRegionSetIterator it(worker_slices[wid]);
    ShenandoahHeapRegion* r = it.next();
    while (r != NULL) {
      size_t idx = r->index();
      assert(ShenandoahPrepareForCompactionTask::is_candidate_region(r), err_msg("Sanity: " SIZE_FORMAT, idx));
      assert(!map.at(idx), err_msg("No region distributed twice: " SIZE_FORMAT, idx));
      map.at_put(idx, true);
      r = it.next();
    }
  }
  ...
  for (size_t rid = 0; rid < n_regions; rid++) {
    ...
    assert(is_distributed || !is_candidate, err_msg("All candidates are distributed: " SIZE_FORMAT, rid));
  }
#endif
}
...
void ShenandoahMarkCompact::phase3_update_references() {
  ...
  uint nworkers = workers->active_workers();
  {
    ...
    ShenandoahRootAdjuster rp(ShenandoahPhaseTimings::full_gc_adjust_roots);
...
void ShenandoahMarkCompact::compact_humongous_objects() {
  ...
  for (size_t c = heap->num_regions(); c > 0; c--) {
    ...
    if (r->is_humongous_start()) {
      ...
      assert(r->is_stw_move_allowed(), err_msg("Region " SIZE_FORMAT " should be movable", r->index()));
